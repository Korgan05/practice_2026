...
- **Задача 8** — управление «Договорами» (создание/редактирование/удаление):
  номер, предмет, контрагент (ссылка на справочник), сумма/валюта, статус
  (черновик/действует/завершён), даты заключения/начала/окончания, комментарий;
  поиск по номеру/предмету (`/api/contracts`).
- **Задача 9** — поле «Документы» в договоре: множественный выбор файлов с тегом
  «Договор» (M:M `contract_documents`) и загрузка нового файла с автотегом «Договор»
  прямо из карточки договора с прикреплением. Тег «Договор» гарантируется миграцией.
- **Задача 10** — раздел «Проекты» (создание/редактирование/удаление): наименование,
  код/шифр, описание, статус, даты, руководитель (ссылка на пользователя), связь с
  договорами (M:M). Эндпоинт `/api/users/brief` для выбора руководителя.
- **Задача 11** — согласование документов договора: участники договора (M:M
  `contract_participants`), согласование каждого прикреплённого документа участниками
  (`document_approvals`), модальное окно с матрицей «документы × участники» и кнопкой
  «Согласовать», прогресс «согласовали N из M».
- **Задача 12** — флаг профиля «Всегда добавлять в список согласования»: согласовывать
  документы могут только пользователи с этим флагом (Директора/замы/начальники);
  участники-исполнители выполняют ознакомление через кнопку «Ознакомиться» →
  модальное окно подтверждения → «Подтверждаю»/«Отмена». В карточке договора видны
  раздельно статусы согласования и ознакомления по каждому документу.
